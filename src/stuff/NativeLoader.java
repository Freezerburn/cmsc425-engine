 ...
 * library code is expected, or a {@link UnsatisfiedLinkError}s will not be prevented. The loader can be disabled
...
public class NativeLoader {
	...
	/**
	 * Property to disable Native loading. Set to true and the Loader will not load any libraries regardless of any other setting.
	 */
	...
	public static final void load(){
			if(librariesAreLoadable()){
				...
				System.out.println("Loading native libraries automatically");
				...
				try {
					...
					System.err.println("Unable to load native libraries. They must be set manually with '-Djava.library.path'");
                    e.printStackTrace();
					//We failed. We shouldn't kill the application however, linking *may* have succeeded because of user manually setting location
				}
			}else {
				System.out.println("Not automatically loading native libraries.");
			}
	}
		...
		if(alreadyLoaded && outputDirectoryHandle != null){
			...
			for(int iter = 0; iter < children.length; iter++){
				...
				try{
					...
					System.err.println("Unable to delete temp file from native library temp folder");
					successFlag = false;
				}
			...
			try{
				...
				System.err.println("Unable to delete native library temp folder");
				successFlag = false;
			}
	...
	private static final boolean librariesAreLoadable(){
		...
		if(System.getProperty(DISABLE_AUTO_LOADING_PROPERTY, "false").equalsIgnoreCase("true")){
			return false;//Respect user configuration to disable loading, but default to loading
		}
	...
	private static final boolean testLibraryAvailable(String libName,String libName64) throws SecurityException{
		try{
			...
			System.out.println("Library " + libName + " is already available.");
			return true;
		}catch(UnsatisfiedLinkError e){//library not found
			System.out.println("Library " + libName + " is not already available.");
			if(libName64 != null){
	 ...
	 * {@link #writeResourceToFile(java.io.InputStream, java.io.File)} for output onto the file system.
	 * 
	 * @param platform An integer representing the platform.
	 * @throws java.io.IOException If writing the stream to file fails
	 */
...
//                nativeResourceJar = new JarFile(new File(WINDOWS_NATIVE_JAR));
        ...
        URL jarResource = NativeLoader.class.getResource("/" + systemJar);
        if(jarResource == null) {
            System.out.println("Loading normally");
            nativeResourceJar = new JarFile(new File(systemJar));
        }
        else {
            System.out.println("Loading from inside a JAR");
//                    System.out.println(NativeLoader.class.getResource("/natives-mac.jar"));
            String jarResourceString = jarResource.toString();
            jarResourceString = jarResourceString.substring(0, jarResourceString.lastIndexOf("!"));
            jarResourceString = jarResourceString.substring(jarResourceString.lastIndexOf(":") + 1);
//                    System.out.println(jarResourceString);
            JarFile container = new JarFile(jarResourceString);
            JarEntry nativesJar = container.getJarEntry(systemJar);
            InputStream jarStream = container.getInputStream(nativesJar);
            File tempjar = File.createTempFile(systemJar, ".jar");
            tempjar.deleteOnExit();
            writeResourceToFile(jarStream, tempjar);
            jarStream.close();
            nativeResourceJar = new JarFile(tempjar);
        }
		...
		for(int iter = 0; iter < libraries.length;iter++){//Loop through all the libraries and load them
			...
			try{
                JarEntry entry = nativeResourceJar.getJarEntry(prefix + fileName);
				resourceStream = nativeResourceJar.getInputStream(entry);
...
//		System.out.println("LIBRARY_SYSTEM_PROPERTY: " + outputDir);
	}
	 ...
	 * @throws java.io.IOException In the event that the write fails for any reason, or if either input is null.
	 */
